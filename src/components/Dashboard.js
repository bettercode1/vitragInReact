...
import { Container, Row, Col, Card, Button, Badge, ProgressBar, Tab, Tabs, Table, Form, Dropdown } from 'react-bootstrap';
...
import VitragPDFGenerator from '../utils/pdfGenerator';
...
const Dashboard = () => {
  ...
  const achievements = [
    { id: 1, name: 'First Test', icon: faVial, requirement: 1, completed: 1, color: 'success', unlocked: true },
    { id: 2, name: '5 Tests', icon: faMicroscope, requirement: 5, completed: 0, color: 'secondary', unlocked: false },
    { id: 3, name: '10 Tests', icon: faFlask, requirement: 10, completed: 0, color: 'secondary', unlocked: false },
    { id: 4, name: '25 Tests', icon: faCog, requirement: 25, completed: 0, color: 'secondary', unlocked: false }
  ];
  ...
  // PDF Generation function using JavaScript
  const generatePDF = async () => {
    setIsGeneratingPDF(true);
    try {
      // Mock data for PDF generation - matching the Python file structure
      const testData = {
        test_request: {
          id: 1,
      ...
      // Save the PDF
      const fileName = `Test_Report_${testData.test_request.job_number}.pdf`;
