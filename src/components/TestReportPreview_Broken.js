...
const TestReportPreview = () => {
  ...
  useEffect(() => {
    const fetchTestData = async () => {
      ...
      try {
        ...
        // The new PDF data structure already has parsed JSON fields
        const mainTest = data.main_test || {};
        const strengthData = mainTest.test_results_json || {};
        const parsedObservationsData = mainTest.observations_json || {};
        ...
        console.log('  observationsData:', parsedObservationsData);
        ...
        const transformedData = {
          ...
          cubeTests: [{
            ...
            testResults: parsedObservationsData.testRows?.map((row, i) => {
            ...
            // If no testRows from observations API, create from concrete test data
            if (testResults.length === 0) {
              const quantity = ct.quantity || ct.num_of_cubes || 3;
              for (let i = 0; i < quantity; i++) {
                const area = ct.dimension_length && ct.dimension_width 
                  ? (ct.dimension_length * ct.dimension_width).toFixed(3)
                  : '22500.000';
            ...
            return {
              ...
              testedBy: observationsData.formData?.testedBy || ct.testedBy || ct.tested_by || 'N/A',
              checkedBy: observationsData.formData?.checkedBy || ct.checkedBy || ct.checked_by || 'N/A',
              verifiedBy: observationsData.formData?.verifiedBy || ct.verifiedBy || ct.verified_by || 'Mr. P A Sanghave',
              ...
              testRemarks: observationsData.formData?.testRemarks || ct.testRemarks || ct.test_remarks || ''
            };
          }),
          strengthData: strengthData,
          observationsData: parsedObservationsData
        };
  ...
  const handleViewObservationSheet = () => {
    ...
    // Get tested/checked/verified by from testData.observationsData.formData (from test observations API)
    const formData = testData.observationsData?.formData || {};
    console.log('🔍 FULL formData object:', formData);
    console.log('🔍 formData.testedBy:', formData.testedBy);
    console.log('🔍 formData.checkedBy:', formData.checkedBy);
    console.log('🔍 formData.verifiedBy:', formData.verifiedBy);
    console.log('🔍 formData.testRemarks:', formData.testRemarks);
    
    // Use the signature data from test observations API
    params.append('tested_by_name', formData.testedBy || '');
    params.append('checked_by_name', formData.checkedBy || '');
    params.append('verified_by_name', formData.verifiedBy || '');
    
    // Add dates (use testing date as default for all signatures)
    const signatureDate = testData.cubeTests?.[0]?.testingDate || new Date().toLocaleDateString('en-GB');
    ...
    // Add remarks
    params.append('remarks', formData.testRemarks || '');
    
    // Add average strength from database
    const avgStrength = testData.cubeTests?.[0]?.averageStrength || cubeTest.averageStrength || testData.averageStrength || '';
    params.append('average_strength', avgStrength);
    console.log('📊 Average Strength for observation sheet:', avgStrength);
    console.log('🔍 testData.cubeTests[0].averageStrength:', testData.cubeTests?.[0]?.averageStrength);
    
    console.log('📋 Observation Sheet URL Params:', {
      tested_by_name: formData.testedBy,
      checked_by_name: formData.checkedBy,
      verified_by_name: formData.verifiedBy,
  ...
  const handleViewPDF = () => {
    ...
    params.append('blocks_condition', cubeTest.cubeCondition || testData.cubeCondition || '');
    params.append('condition_of_specimen', cubeTest.cubeCondition || testData.cubeCondition || '');
    params.append('condition_of_sample', cubeTest.cubeCondition || testData.cubeCondition || '');
    ...
    params.append('machine_used', cubeTest.machineUsed || testData.machineUsed || '');
    params.append('machine_used_for_testing', cubeTest.machineUsed || testData.machineUsed || '');
    params.append('location_of_testing', cubeTest.locationNature || testData.locationNature || '');
    params.append('location_structure', cubeTest.locationNature || testData.locationNature || '');
    params.append('type_of_specimen', cubeTest.sampleDescription || testData.sampleDescription || '');
    params.append('environmental_conditions', cubeTest.environmentalConditions || testData.environmentalConditions || '');
    params.append('capacity_range', cubeTest.capacityRange || testData.capacityRange || '');
    params.append('calibration_due_date', cubeTest.calibrationDueDate || testData.calibrationDueDate || '');
